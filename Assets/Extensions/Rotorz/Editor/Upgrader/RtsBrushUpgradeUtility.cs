...
public sealed class RtsBrushUpgradeUtility {
	...
	private void UpgradeAtlasBrush(RtsBrushAssetRecordWrapper record) {
		...
		BoxCollider automaticCollider = oldBrush.GetComponent<Collider>() as BoxCollider;
		...
		bool attachPrefab = ( oldTransform.childCount > 0 )
			|| ( !newBrush.addCollider && oldBrush.GetComponent<Collider>() != null )
			|| ( componentCount != 4)
			;
		if (attachPrefab) {
			...
			if (newBrush.addCollider)
				Object.DestroyImmediate(attachment.GetComponent<Collider>());
			
			// Remove mesh filter and renderer components.
			Object.DestroyImmediate(attachment.GetComponent<Renderer>());
